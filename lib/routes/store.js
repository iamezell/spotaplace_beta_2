...
exports.index = function(req, res){
	...
	store.findOne({userId:userId}, function(err, myStore){
			if(!myStore){
				...
				//get email of the vendor
				var email = myStore.email;
					...
					auctions.findOne({_id:id}, function(err, theAuction){
						if(!theAuction){
							...
							store.update({ _id:myStore._id },
					     	...
					     	myStore.save();
					     	//now that we removed the auction lets email the participants
					     	// send mail with defined transport object
					     	console.log("this is the email for the participants")
		 				var mailOptions = {
						    from: "Spotaplace <ezell@spotaplace.com>", // sender address
						    to: email, // list of receivers
						    subject: "Auction for blank closed", // Subject line
						    text: "Thank you for participating in our Auction", // plaintext body
						    html: 'Thank you for participating in our Auction' // html body
						}
