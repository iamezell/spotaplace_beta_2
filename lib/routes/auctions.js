...
exports.index = function(req, res){
	...
	console.log('auction being called');

	//check if the page has already been rendered to start socket.io connection event handler
	if(!runFlag){
		// it has not been run lets run it
		startSocket(auctionId);
		console.log('the auction is is');
	...
	if(POR == 'vendor'){
		...
		auction.findOne({_id:auctionId}, function(err , theAuction){
			...
			res.cookie('auction_id', theAuction._id, { maxAge: 900000, httpOnly: false});
...
function startSocket(auctionid){
global.io.on('connection', function(socket){ 
	//console.log('connection is made');
	//define the room

	
	//blacklisted functionality comming soon
		...
		auction.findOne({_id:auctionID}, function(err, auctionObj){
			store.findOne({_id:vendorID}, function(err, store){
				////console.log(store);
				 ...
				 // we should get more vendor inforatioon tharn we have at this junction
				 global.io.sockets.in(auctionid).emit('bidBoxReturn', {'auctionID':auctionID, 'vendorID':vendorID,'description':description, 'name':storeName })
				 //console.log(global.io.sockets.clients(auctionRoom));
				});
	...
	socket.on('disconnect', function(){
		...
		//just in case not sure this even works
